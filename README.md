...
What this covers
Loading and exploring the count data
Basic stats- mean and median expression per sample
Heatmap of gene expression
PCA plot- to see how similar/different my samples are
Finding DEGs using a t-test
Volcano plot of the results
Violin plot for a single gene of interest
...
├── analysis.py               # the main script (Day 5 version below)
└── DEG_results.csv           # output: significant genes found by the t-test

Note: as written, the script displays each plot on screen with plt.show()- it doesn't save the plots as image files. If you want to keep copies of the plots (e.g. to show on GitHub), see Saving the plots as image files below.
...
Simple explanation: this just opens my count table (genes as rows, samples as columns) into a table I can work with in Python.
